  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateCsTenantUserFunc(scope.row)">变更</el-button>
...
import {
  createCsTenantUser,
  deleteCsTenantUser,
  deleteCsTenantUserByIds,
  updateCsTenantUser,
  findCsTenantUser,
  getCsTenantUserList
} from '@/api/csTenantUser'
...
const getTableData = async() => {
  const table = await getCsTenantUserList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteCsTenantUserByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateCsTenantUserFunc = async(row) => {
    const res = await findCsTenantUser({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.recsTenantUser
...
// 删除行
const deleteCsTenantUserFunc = async (row) => {
    const res = await deleteCsTenantUser({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createCsTenantUser(formData.value)
                  break
                case 'update':
                  res = await updateCsTenantUser(formData.value)
                  break
                default:
                  res = await createCsTenantUser(formData.value)
                  break
