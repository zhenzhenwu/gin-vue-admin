  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateCsOperatorUsersFunc(scope.row)">变更</el-button>
...
import {
  ...
  deleteCsOperatorUsers,
  deleteCsOperatorUsersByIds,
  updateCsOperatorUsers,
  findCsOperatorUsers,
  getCsOperatorUsersList
...
const getTableData = async() => {
  const table = await getCsOperatorUsersList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteCsOperatorUsersByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateCsOperatorUsersFunc = async(row) => {
    const res = await findCsOperatorUsers({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteCsOperatorUsersFunc = async (row) => {
    const res = await deleteCsOperatorUsers({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
